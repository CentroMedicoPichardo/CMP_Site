// src/app/api/cursos/[id]/analytics/route.ts
...
async function generarHistorialSimulado(cursoId: number) {
  try {
    ...
    const diasDesdeInicio = Math.max(
      0,
      Math.floor(
        (hoy.getTime() - fechaInicio.getTime()) / (1000 * 3600 * 24)
      )
    );
    ...
    for (let i = 0; i <= Math.min(diasDesdeInicio, 30); i++) {
      ...
      ocupadosAcumulados = Math.min(
        Math.floor(cuposOcupados * progreso),
        cuposOcupados
      );
...
async function calcularTasaConversion(cursoId: number) {
  try {
    ...
    const totalInscripciones =
      Number(inscripciones[0]?.total_inscripciones) || 0;
...
function calcularTendencia(historial: any[]) {
  ...
  const promedioPrimera =
    primeraMitad.reduce((sum, d) => sum + d.ocupados, 0) /
    primeraMitad.length;

  const promedioSegunda =
    segundaMitad.reduce((sum, d) => sum + d.ocupados, 0) /
    segundaMitad.length;
...
export async function GET(
  ...
  try {
    ...
    console.error("Error en analytics de curso:", error);
